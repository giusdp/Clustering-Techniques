...
def em_clustering(x_d, y_d, k, iters):
    global X
    X = x_d
    global Y
    Y = y_d
...
class GMM:
    ...
    def plot(self):
        ...
        # fig3 = plt.figure(figsize=(10, 10))
        # ax2 = fig3.add_subplot(111)
        # ax2.scatter(self.X[:, 0], self.X[:, 1], s=15, cmap='rainbow')
        # for m, c in zip(self.mu, self.cov):
        #     multi_normal = multivariate_normal(mean=m, cov=c)
        #     ax2.contour(np.sort(self.X[:, 0]), np.sort(self.X[:, 1]),
        #                 multi_normal.pdf(self.XY).reshape(len(self.X), len(self.X)), colors='black', alpha=0.3)
        #     ax2.scatter(m[0], m[1], zorder=10, s=100, cmap='rainbow')
        ax.set_title('EM - Gaussian Mixture')